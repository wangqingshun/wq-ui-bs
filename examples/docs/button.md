...
设置```icon```属性即可，icon 的列表可以参考 Wl-UI 的 icon 组件，也可以设置在文字右边的 icon ，只要使用```i```标签即可，可以使用自定义图标。
<div class="demo-block">
...
<div class="demo-block">
  ...
  <div class="m-10">
    <w-button round>默认尺寸</w-button>
    <w-button size="medium" round>中等按钮</w-button>
    <w-button size="small" round>小型按钮</w-button>
  </div>
</div>
